...
import ani.himitsu.databinding.BottomSheetSearchFilterBinding
...
class SearchFilterBottomDialog : BottomSheetDialogFragment() {
    ...
    private fun resetSearchFilter() {
        ...
        binding.sortByFilter.setImageResource(R.drawable.round_filter_alt_24)
        startBounceZoomAnimation(binding.sortByFilter)
        activity.result.countryOfOrigin = null
        binding.countryFilter.setImageResource(R.drawable.ic_globe_search_googlefonts)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
            ...
            popupMenu.setOnMenuItemClickListener { menuItem ->
                when (menuItem.itemId) {
                    R.id.sort_by_score -> {
                        ...
                        binding.sortByFilter.setImageResource(R.drawable.round_area_chart_24)
                        startBounceZoomAnimation()
                    }

                    R.id.sort_by_popular -> {
                        activity.result.sort = AniList.sortBy[1]
                        binding.sortByFilter.setImageResource(R.drawable.round_filter_hdr_24)
                        startBounceZoomAnimation()
                    }

                    R.id.sort_by_trending -> {
                        activity.result.sort = AniList.sortBy[2]
                        binding.sortByFilter.setImageResource(R.drawable.round_auto_graph_24)
                        startBounceZoomAnimation()
                    }

                    R.id.sort_by_recent -> {
                        activity.result.sort = AniList.sortBy[3]
                        binding.sortByFilter.setImageResource(R.drawable.round_new_releases_24)
                        startBounceZoomAnimation()
                    }

                    R.id.sort_by_a_z -> {
                        activity.result.sort = AniList.sortBy[4]
                        binding.sortByFilter.setImageResource(R.drawable.round_filter_list_24)
                        startBounceZoomAnimation()
                    }

                    R.id.sort_by_z_a -> {
                        activity.result.sort = AniList.sortBy[5]
                        binding.sortByFilter.setImageResource(R.drawable.round_filter_list_24_reverse)
                        startBounceZoomAnimation()
                    }

                    R.id.sort_by_user_score -> {
                        activity.result.sort = AniList.sortBy[6]
                        binding.sortByFilter.setImageResource(R.drawable.round_assist_walker_24)
                        startBounceZoomAnimation()
                    }
                }
                true
            ...
            popupMenu.setOnMenuItemClickListener { menuItem ->
                when (menuItem.itemId) {
                    ...
                    R.id.country_china -> {
                        ...
                        binding.countryFilter.setImageResource(R.drawable.ic_globe_china_googlefonts)
                        startBounceZoomAnimation(binding.countryFilter)
                    }

                    R.id.country_south_korea -> {
                        activity.result.countryOfOrigin = "KR"
                        binding.countryFilter.setImageResource(R.drawable.ic_globe_south_korea_googlefonts)
                        startBounceZoomAnimation(binding.countryFilter)
                    }

                    R.id.country_japan -> {
                        activity.result.countryOfOrigin = "JP"
                        binding.countryFilter.setImageResource(R.drawable.ic_globe_japan_googlefonts)
                        startBounceZoomAnimation(binding.countryFilter)
                    }

                    R.id.country_taiwan -> {
                        activity.result.countryOfOrigin = "TW"
                        binding.countryFilter.setImageResource(R.drawable.ic_globe_taiwan_googlefonts)
                        startBounceZoomAnimation(binding.countryFilter)
                    }
                }
                true
        ...
        if (activity.result.type == "ANIME") {
            binding.searchYear.setText(activity.result.seasonYear?.string)
        } else {
            binding.searchYear.setText(activity.result.startYear?.string)
        }
