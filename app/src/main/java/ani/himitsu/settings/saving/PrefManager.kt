...
import ani.himitsu.media.anime.ExoplayerView.Companion.media
import ani.himitsu.settings.saving.internal.Compat
...
object PrefManager {
    ...
    fun hasCustomVal(key: String): Boolean {
        return irrelevantPreferences?.all?.contains(key) == true
    }
    ...
    fun clearMediaVals(mediaId: String) {
        CoroutineScope(Dispatchers.IO).launch {
            irrelevantPreferences!!.all.entries.map {
                async {
                    if (it.key.startsWith("${media.id}_")) {
                        with(irrelevantPreferences!!.edit()) {
                            remove(it.key)
                            apply()
                        }
    ...
    fun removeKey(location: Location, name: String) {
        getPrefLocation(location).let {
            if (it.contains(name)) it.edit().remove(name).apply()
        }
    }
}