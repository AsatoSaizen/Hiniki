...
class NovelExtensionManager(private val context: Context) {
    ...
    suspend fun findAvailableExtensions() {
        val extensions: List<NovelExtension.Available> = try {
            api.findNovelExtensions().sortedBy { it.name.lowercase() }
        } catch (e: Exception) {
            Logger.log("Error finding extensions: ${e.message}")
            snackString("Failed to get Novel extensions list")
            emptyList()
        }
    ...
    private val _availableNovelPluginsFlow =
        MutableStateFlow(emptyList<NovelPlugin.Available>())
    val availablePluginsFlow = _availableNovelPluginsFlow.asStateFlow()

    private var availableNovelPluginsSourcesData: Map<Long, PluginSourceData> = emptyMap()

    private fun setupAvailableNovelPluginsSourcesDataMap(novelPlugins: List<NovelPlugin.Available>) {
        if (novelPlugins.isEmpty()) return
        availableNovelPluginsSourcesData = novelPlugins
            .flatMap { ext -> ext.sources.map { it.toPluginSourceData() } }
            .associateBy { it.id }
    }

    suspend fun findAvailablePlugins() {
        val plugins: List<NovelPlugin.Available> = try {
            api.findNovelPlugins().sortedBy { it.name.lowercase() }
        } catch (e: Exception) {
            Logger.log("Error finding plugins: ${e.message}")
            snackString("Failed to get Novel plugin list")
            emptyList()
        }
