...
import ani.himitsu.util.ColorEditor
    ...
    companion object {
        ...
        fun buildChart(
            ...
            scrollPos?.let {
                ...
                aaOptions.chart?.scrollablePlotArea?.minWidth((metrics.widthPixels.toFloat() / metrics.density) * (namesMax.toFloat() / 18f))
            }
                ...
                if (names[i] is Number) {
                    ...
                    element.dataLabels(
                        AADataLabels()
                            .enabled(false)
                            .format("{point.y}")
                            .backgroundColor(AAColor.rgbaColor(255, 255, 255, 0f))
                    )
                } else {
                    element.x(i)
