...
object CommentsAPI {
    ...
    private suspend fun getUserDetails(client: OkHttpClient? = null): User? {
        ...
        if (json.code == 200) {
            ...
            isBanned = parsed.user.isBanned == true
            isAdmin = parsed.user.isAdmin == true
            isMod = parsed.user.isMod == true
    ...
    suspend fun fetchAuthToken(context: Context, client: OkHttpClient? = null) {
        ...
        if (tokenExpiry < System.currentTimeMillis() + tokenLifetime) {
            ...
            if (commentResponse != null) {
                ...
                isBanned = commentResponse.user.isBanned == true
                isAdmin = commentResponse.user.isAdmin == true
                isMod = commentResponse.user.isMod == true
    ...
    private fun errorMessage(reason: String) {
        Logger.log(reason)
        if (isOnline) snackString("CommentsAPI: $reason")
    }
