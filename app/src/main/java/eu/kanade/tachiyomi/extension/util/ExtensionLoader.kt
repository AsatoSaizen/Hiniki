...
import ani.himitsu.parsers.novel.NovelLoadResult
...
internal object ExtensionLoader {
    ...
    var trustedSignaturesManga = mutableSetOf<String>() +
            preferences.trustedSignatures().get() + officialSignatureManga

    // dan's key
    private const val officialSignatureNovel =
    ...
    fun loadAnimeExtensions(context: Context): List<AnimeLoadResult> {
        ...
        return extPkgs.asyncMap { loadAnimeExtension(context, it.packageName, it) }
    }
    ...
    fun loadMangaExtensions(context: Context): List<MangaLoadResult> {
        ...
        // Load each extension concurrently and wait for completion
        return extPkgs.asyncMap { loadMangaExtension(context, it.packageName, it) }
    }
    ...
    fun loadNovelExtensions(context: Context): List<NovelLoadResult> {
        ...
        // Load each extension concurrently and wait for completion
        return extPkgs.asyncMap { loadNovelExtension(context, it.packageName, it) }
    }
    ...
    private fun loadAnimeExtension(
        ...
        if (signatureHash == null) {
            ...
            return AnimeLoadResult.Error
            /*  temporarily disabling signature check TODO: Refactor
        } else if (signatureHash !in trustedSignaturesAnime) {
            val extension = AnimeExtension.Untrusted(
                extName, pkgName, versionName, versionCode, libVersion, signatureHash
            )
            Logger.log("Extension $pkgName isn't trusted")
            return AnimeLoadResult.Untrusted(extension)
        */
        } else {
            // Injekt.get<AnimeExtensionManager>().trustSignature(signatureHash)
        }
        ...
        val lang = sources.filterIsInstance<AnimeCatalogueSource>().map { it.lang }.toSet().let {
            when (it.size) {
                0 -> ""
                1 -> it.first()
                else -> "all"
    ...
    private fun loadMangaExtension(
        ...
        */
            ...
            // Injekt.get<MangaExtensionManager>().trustSignature(signatureHash)
        }
